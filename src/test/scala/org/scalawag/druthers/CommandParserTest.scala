...
class CommandParserTest extends ParserTest {
  ...
  test("keywords only - too few arguments") {
    fail[ABC]("8") match {
      case Seq(MissingArgument(arg)) => arg.name should be ("b")
    }
  }

  test("keywords only - way too few arguments") {
    fail[ABC]("") match {
      case Seq(MissingArgument(arg)) => arg.name should be ("a") // TODO: should this report both missing arguments?
    }
  }
  ...
  test("greedy Option saves one at the end for required argument") {
    parseOf[GreedyOptionWithRequired]("b") should be (GreedyOptionWithRequired(None,"b"))
  }
  ...
  test("default parameters") {
    ...
    parser.parse(Nil) should be (DefaultParameters())
  }

  test("default parameters are greedy") {
    val parser = new CommandParser[DefaultParameters]

    parser.parse(split("8")) should be (DefaultParameters("8"))
  }
  ...
  test("default parameters are greedy but will give up parameters to meet requirements") {
    ...
    parser.parse(split("8")) should be (DefaultParametersWithRequired(b = 8))
  }
  ...
  private[this] def fail[T:TypeTag](args:String,config:ParserConfiguration = null) = {
    intercept[UsageException]((new CommandParser[T](config)).parse(split(args))).errors
  }
